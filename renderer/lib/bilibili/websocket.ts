      ...
      case MessageOP.SEND_MSG_REPLY: {
        ...
        switch (result.ver) {
          ...
          case WsBodyVer.BROTLI: {
             // Brotli not supported in pure browser without wasm yet, skipping or use polyfill if needed
             // For now, Bilibili usually supports deflate if we don't request brotli, or we need to add brotli-wasm
             log.warn('Brotli compression received but not implemented in browser version yet')
             break
          }
          default: {
...
export class BiliWebSocket {
  ...
  private reconnect() {
    ...
    setTimeout(() => {
      ...
      log.info('Reconnecting to room websocket')
      this.Connect()
    }, 5000)
  }
}
...
class BiliInternalWebSocket {
  ...
  public Connect() {
    ...
    this._ws.binaryType = 'arraybuffer' // Important for browser WebSocket
    ...
    this._ws.onopen = () => {
      ...
      const auth_info = {
        ...
        protover: 2, // Use 2 for deflate, 3 is brotli
        type: 2,
        platform: 'web',
      ...
      this._heartbeat_task = setInterval(() => {
        ...
        this._ws.send(heart_msg.GetBuffer())
      }, 30 * 1000) // Bilibili heartbeat is 30s
    }
    ...
    this._ws.onerror = (err) => {
      ...
      // this will trigger close event, so we don't need to handle it here
    }
  }
